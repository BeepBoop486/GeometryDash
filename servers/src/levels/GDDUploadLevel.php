	...
	if (isset($_POST["secret"])) {
		...
		if ($secret == GAME_REQ_SECRET) {
			if (isset($_POST["lvlname"]) && isset($_POST["lvlcnt"])) {
				...
				if($lvlname != "" && $lvlcnt != "") {
					$query = $conn->query("INSERT INTO levels(id, lvlname, lvlcnt, uploader, rate, stars, downloads, likes, dislikes) VALUES(NULL, '$lvlname', '$lvlcnt', '$uploadern', NULL,NULL, NULL,NULL,NULL)");
					if ($query) {
						echo "1";
					} else {
						echo "-1";
